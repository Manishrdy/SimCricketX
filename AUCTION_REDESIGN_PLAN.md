...
- [x] **Phase 8.2** (refactor) — Shared ownership guards. Extracted the ~30-line `_own_season_or_404` / `_own_season_team_or_404` / `_own_category` / `_own_ap` / `_own_auction` helpers (previously duplicated across `league_routes.py`, `auction_routes.py`, and `auction_realtime.py`) into `routes/_auction_guards.py`. `make_guards(...)` is a model-binding factory that returns a namespace; each route file aliases the relevant methods to the existing local names so callsites are unchanged. `current_user` is resolved from `flask_login` per call.
